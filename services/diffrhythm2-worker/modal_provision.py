...
@app.function(image=smoke_image,gpu="L40S",secrets=[secret],
              volumes={MODEL_MOUNT:models,SMOKE_MOUNT:smoke},timeout=3600)
def smoke_real_audio():
 import json,os,subprocess
 fixture="/var/lib/diffrhythm2/smoke/golden-30s.wav"
 subprocess.run(["ffmpeg","-nostdin","-y","-ss","30","-t","30","-i","/smoke/golden.mp3",
                 "-ac","1","-ar","44100","-c:a","pcm_s16le",fixture],
                check=True,capture_output=True)
 try:
  completed=subprocess.run(["/opt/diffrhythm2-venv/bin/python","smoke.py"],cwd="/app",
                           env={**os.environ,**environment(False),"DIFFRHYTHM2_SMOKE_AUDIO":fixture},
                           capture_output=True,text=True)
  if completed.returncode:
   diagnostic=" ".join(completed.stderr.split())[-4000:]
   raise RuntimeError(f"DiffRhythm 2 real smoke failed: {diagnostic}")
  return json.loads((__import__("pathlib").Path(MODEL_MOUNT)/"smoke-proof.json").read_text())
 finally:
  models.commit()
  smoke.commit()