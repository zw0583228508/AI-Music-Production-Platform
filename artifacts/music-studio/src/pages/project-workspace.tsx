...
export default function ProjectWorkspace() {
  ...
  const handleGenerate = () => {
    ...
    generateArrangement.mutate({
      ...
      onSuccess: () => {
        toast({ title: "Generation Started", description: "Creating new arrangement candidates." });
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
            ...
            <TabsContent value="candidates" className="flex-1 m-0 p-6 min-h-0 overflow-auto">
               <div className="max-w-3xl mx-auto">
                 ...
                 {activeArrangement?.status === 'ready' ? (
                   <div className="space-y-4">
                     {[1,2,3].map(i => (
                       <Card key={i} className="group hover:border-primary/50 transition-colors shadow-sm cursor-pointer">
                         <CardContent className="p-4 flex items-center justify-between">
                           <div className="flex items-center gap-4">
                             <Button variant="secondary" size="icon" className="h-10 w-10 rounded-full group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                               <Play className="h-5 w-5 ml-1" />
                             </Button>
                             <div>
                               <div className="font-semibold">Candidate {String.fromCharCode(64 + i)}</div>
                               <div className="text-xs text-muted-foreground">Score: {Math.floor(Math.random() * 20 + 80)}/100</div>
                             </div>
                           </div>
